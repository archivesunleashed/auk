...
# Methods for PDF Information Extrator.
class PdfsJob < ApplicationJob
  queue_as :pdfs

  after_perform do |job|
    spark_name = 'PDF information extraction'
  ...
  def perform(user_id, collection_id)
    ...
    Collection.where('user_id = ? AND collection_id = ?',
      ...
      collection_derivatives = collection_path + c.user_id.to_s +
                               '/derivatives'
      FileUtils.rm_rf collection_derivatives + '/pdfs'
      ...
      pdfs_job = spark_home +
                 ...
                 '/aut-' +
                 aut_version +
                 '-fatjar.jar --extractor PDFInformationExtractor --input ' +
                 collection_warcs +
                 ' --output ' +
                 collection_derivatives +
                 '/pdfs --partition 1 2>&1 | tee ' +
                 spark_log +
                 '/' +
                 ...
                 '.log'
      logger.info 'Executing: ' + pdfs_job
      system(pdfs_job)
      success = collection_derivatives + '/pdfs/_SUCCESS'
      combine = 'find ' +
                collection_derivatives +
                '/pdfs -iname "part*" -type f -exec cat {} > ' +
                collection_derivatives +
                '/pdfs/' +
                c.collection_id.to_s +
                '-pdfs.csv \;'
      cleanup = 'find ' +
                collection_derivatives +
                '/pdfs -iname "part*" -type f -delete'
      hidden = 'find ' +
               collection_derivatives +
               '/pdfs -iname ".*" -type f -delete'
      if File.exist?(success)
        system(combine)
        FileUtils.rm(success)
        system(cleanup)
        system(hidden)
        logger.info 'Executed: PDF information cleanup.'
      else
        UserMailer.notify_collection_failed(c.user_id.to_s,
                                            c.collection_id.to_s).deliver_now
        raise 'PDF information spark job failed.'
      end
    end
  end
end
