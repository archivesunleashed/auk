...
class WebgraphJob < ApplicationJob
  queue_as :webgraph

  after_perform do |job|
    spark_name = 'Web graph extraction'
  ...
  def perform(user_id, collection_id)
    ...
    Collection.where('user_id = ? AND collection_id = ?',
      ...
      collection_derivatives = collection_path + c.user_id.to_s +
                               '/derivatives'
      FileUtils.rm_rf collection_derivatives + '/webgraph'
      ...
      webgraph_job = spark_home +
                     ...
                     '/aut-' +
                     aut_version +
                     '-fatjar.jar --extractor WebGraphExtractor --input ' +
                     collection_warcs +
                     ' --output ' +
                     collection_derivatives +
                     '/webgraph --partition 1 2>&1 | tee ' +
                     spark_log +
                     '/' +
                     ...
                     '.log'
      logger.info 'Executing: ' + webgraph_job
      system(webgraph_job)
      success = collection_derivatives + '/webgraph/_SUCCESS'
      combine = 'find ' +
                collection_derivatives +
                '/webgraph -iname "part*" -type f -exec cat {} > ' +
                collection_derivatives +
                '/webgraph/' +
                c.collection_id.to_s +
                '-webgraph.csv \;'
      cleanup = 'find ' +
                collection_derivatives +
                '/webgraph -iname "part*" -type f -delete'
      hidden = 'find ' +
               collection_derivatives +
               '/webgraph -iname ".*" -type f -delete'
