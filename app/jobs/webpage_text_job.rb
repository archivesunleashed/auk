...
# Methods for Web Graph Extrator.
  ...
  def perform(user_id, collection_id)
    ...
    Collection.where('user_id = ? AND collection_id = ?',
      ...
      collection_derivatives = collection_path + c.user_id.to_s +
                               '/derivatives'
      FileUtils.rm_rf collection_derivatives + '/webpage_text'
      ...
      webpage_text_job = spark_home +
                         ...
                         '/aut-' +
                         aut_version +
                         '-fatjar.jar --extractor PlainTextExtractor --input ' +
                         collection_warcs +
                         ' --output ' +
                         collection_derivatives +
                         '/webpage_text --partition 1 2>&1 | tee ' +
                         spark_log +
                         '/' +
                         ...
                         '.log'
      logger.info 'Executing: ' + webpage_text_job
      system(webpage_text_job)
      success = collection_derivatives + '/webpage_text/_SUCCESS'
      combine = 'find ' +
                collection_derivatives +
                '/webpage_text -iname "part*" -type f -exec cat {} > ' +
                collection_derivatives +
                '/webpage_text/' +
                c.collection_id.to_s +
                '-webpage_text.csv \;'
      cleanup = 'find ' +
                collection_derivatives +
                '/webpage_text -iname "part*" -type f -delete'
      hidden = 'find ' +
               collection_derivatives +
               '/webpage_text -iname ".*" -type f -delete'
      if File.exist?(success)
        system(combine)
        FileUtils.rm(success)
        system(cleanup)
        system(hidden)
        logger.info 'Executed: Web page text cleanup.'
      else
        UserMailer.notify_collection_failed(c.user_id.to_s,
                                            c.collection_id.to_s).deliver_now
        raise 'Web page text spark job failed.'
      end
    end
  end
end
